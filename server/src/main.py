...
async def function_calling(req: ChatMessage, res: Response):
    try:
        ...
        intent_response = await handler_map["Curve-Intent"].chat_completion(req)
        intent_latency = time.perf_counter() - intent_start_time

        if handler_map["Curve-Intent"].detect_intent(intent_response):
            ...
            try:
                ...
                function_calling_response.metadata = {
                    ...
                    "hallucination": str(handler_map["Curve-Function"].hallucination),
                    "tokens_uncertainty": json.dumps(
                        handler_map["Curve-Function"].hallu_handler.token_probs_map
                    ),
                    "prompt_prefilling": str(
                        handler_map["Curve-Function"].prompt_prefilling
                    ),
                }
                ...
                return {"error": f"[Curve-Function] - {error_message} - {e}"}
                ...
                logger.error(f"Error in chat_completion from `Curve-Function`: {e}")
                res.status_code = 500
                return {"error": f"[Curve-Function] - {e}"}
        # [TODO] Review: define the behavior if `Curve-Intent` doesn't detect an intent
        else:
            return {
        ...
        # [TODO] Review: update how to collect debugging outputs
        # logger.error(f"Error in chat_completion from `Curve-Intent`: {e}")
        logger.error(f"Error in chat_completion /function_calling: {e}")
        res.status_code = 500
        return {"error": f"[Curve-Intent] - {e}"}
...
async def guardrails(req: GuardRequest, res: Response, max_num_words=300):
    try:
        ...
        guard_result = handler_map["Curve-Guard"].predict(req)
