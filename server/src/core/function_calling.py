...
class CurveIntentHandler(CurveBaseHandler):
    def __init__(self, client: OpenAI, model_name: str, config: CurveIntentConfig):
        """
        ...
        Args:
            ...
            config (CurveIntentConfig): The configuration for Curve-Intent.
        """
    ...
    async def chat_completion(self, req: ChatMessage) -> ChatCompletionResponse:
        ...
        if len(req.tools) == 0:
            ...
            logger.info(
                "curve _intent response: %s", json.dumps(model_response.model_dump())
            )
...
class CurveFunctionHandler(CurveBaseHandler):
    def __init__(
        self,
        client: OpenAI,
        model_name: str,
        config: CurveFunctionConfig,
    ):
        """
        ...
        Args:
            client (OpenAI): An OpenAI client instance.
            model_name (str): Name of the model to use.
            config (CurveFunctionConfig): The configuration for Curve-Function
        """
    ...
    async def chat_completion(self, req: ChatMessage) -> ChatCompletionResponse:
        ...
        logger.info(
            f"server => curve _function: request body: {json.dumps(req.model_dump())}"
        )
        ...
        if len(extracted["result"]) and extracted["status"]:
            ...
            if verified["status"]:
                ...
                log_message = f"server <= curve _function: (tool_calls): {json.dumps([tool_call['function'] for tool_call in extracted['result']])}"
        ...
        logger.info(
            f"server <= curve _function: response body: {json.dumps(chat_completion_response.model_dump())}"
        )
