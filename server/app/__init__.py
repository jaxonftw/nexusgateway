...
def start_server():
    ...
    if os.path.exists(PID_FILE):
        print("Server is already running. Use 'server restart' to restart it.")
        sys.exit(1)

    print(f"Starting Curvegw Model Server - Loading some awesomeness, this may take a little time.)")
    process = subprocess.Popen(
        ["uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "51000"],
        start_new_session=True,
        stdout=subprocess.DEVNULL, # Suppress standard output. There is a logger that server prints to
        stderr=subprocess.DEVNULL,  # Suppress standard error. There is a logger that server prints to
    )
...
def stop_server():
    ...
    if not os.path.exists(PID_FILE):
        print("Status: Curvegw Model Server not running")
        return
...
def restart_server():
    ...
    print("Check: Is Curvegw Model Server running?")
    stop_server()
    start_server()
