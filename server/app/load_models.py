...
def load_transformers(model_name=os.getenv("MODELS", "curvelaboratory/bge-large-en-v1.5")):
...
def load_zero_shot_models(
    model_name=os.getenv("ZERO_SHOT_MODELS", "curvelaboratory/deberta-base-nli")
):
    zero_shot_model = {}
    device = get_device()
    if device != "cuda":
