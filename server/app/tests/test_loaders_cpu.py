...
def mock_env():
    ...
    os.environ["MODELS"] = "curvelaboratory/bge-large-en-v1.5"
    os.environ["ZERO_SHOT_MODELS"] = "curvelaboratory/bart-large-mnli"
...
def test_get_embedding_model(mock_tokenizer, mock_automodel, mock_ort_model, mock_env):
    ...
    # Assertions
    assert embedding_model["model_name"] == "curvelaboratory/bge-large-en-v1.5"
    assert mock_tokenizer.called_once_with(
        "curvelaboratory/bge-large-en-v1.5", trust_remote_code=True
    )
    if glb.DEVICE != "cuda":
        assert mock_ort_model.called_once_with(
            "curvelaboratory/bge-large-en-v1.5", file_name="onnx/model.onnx"
        )
    else:
        assert mock_automodel.called_once_with(
            "curvelaboratory/bge-large-en-v1.5", device_map=glb.DEVICE
        )
...
def test_get_zero_shot_model(mock_tokenizer, mock_pipeline, mock_ort_model, mock_env):
    ...
    # Assertions
    assert zero_shot_model["model_name"] == "curvelaboratory/bart-large-mnli"
    assert mock_tokenizer.called_once_with("curvelaboratory/bart-large-mnli")
    if glb.DEVICE != "cuda":
        assert mock_ort_model.called_once_with(
            "curvelaboratory/bart-large-mnli", file_name="onnx/model.onnx"
        )
    else:
...
def test_get_prompt_guard(mock_ov_model, mock_auto_model, mock_tokenizer):
    ...
    prompt_guard = get_prompt_guard(curve _guard_model_type[glb.DEVICE])

    # Assertions
    assert prompt_guard["model_name"] == curve _guard_model_type[glb.DEVICE]
    assert mock_tokenizer.called_once_with(
        curve _guard_model_type[glb.DEVICE], trust_remote_code=True
    )
    if glb.DEVICE == "cpu":
        assert mock_ov_model.called_once_with(
            curve _guard_model_type[glb.DEVICE],
