...
async def test_embedding():
    request_data = {"input": "Test embedding", "model": "curvelaboratory/bge-large-en-v1.5"}
    response = client.post("/embeddings", json=request_data)
    if request_data["model"] == "curvelaboratory/bge-large-en-v1.5":
        assert response.status_code == 200
        assert response.json()["object"] == "list"
...
async def test_zeroshot():
    request_data = {
        ...
        "model": "curvelaboratory/bart-large-mnli",
    }
    response = client.post("/zeroshot", json=request_data)
    if request_data["model"] == "curvelaboratory/bart-large-mnli":
        assert response.status_code == 200
...
async def test_chat_completion():
    async with httpx.AsyncClient(app=app, base_url="http://test") as client:
        request_data = {
            ...
            "metadata": {"x-curve -state": "[]"},  # Assuming metadata is needed
        }
