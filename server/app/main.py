...
from app.commons.constants import embedding_model, zero_shot_model, curve _guard_handler
from app.function_calling.model_utils import (
    chat_completion as curve _function_chat_completion,
)
...
async def guard(req: GuardRequest, res: Response, max_num_words=300):
    ...
    if req.task in ["both", "toxic", "jailbreak"]:
        curve _guard_handler.task = req.task
    else:
    ...
    if len(req.input.split()) < max_num_words:
        guard_result = curve _guard_handler.guard_predict(req.input)
    else:
        ...
        for chunk in chunks:
            chunk_result = curve _guard_handler.guard_predict(chunk)
            guard_result["time"] += chunk_result["time"]
            if chunk_result[f"{curve _guard_handler.task}_verdict"]:
                guard_result[f"{curve _guard_handler.task}_verdict"] = True
                guard_result[f"{curve _guard_handler.task}_sentence"].append(
                    chunk_result[f"{curve _guard_handler.task}_sentence"]
                )
                guard_result[f"{curve _guard_handler.task}_prob"].append(
                    chunk_result[f"{curve _guard_handler.task}_prob"].item()
                )
...
async def chat_completion(req: ChatMessage, res: Response, request: Request):
    try:
        result = await curve _function_chat_completion(req, res)
        return result
