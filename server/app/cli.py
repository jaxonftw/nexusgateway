...
def start_server():
    ...
    if os.path.exists(PID_FILE):
        log.info("Server is already running. Use 'server restart' to restart it.")
        sys.exit(1)
    ...
    process = subprocess.Popen(
        ...
        stdout=subprocess.PIPE,  # Suppress standard output. There is a logger that server prints to
        stderr=subprocess.PIPE,  # Suppress standard error. There is a logger that server prints to
    )
