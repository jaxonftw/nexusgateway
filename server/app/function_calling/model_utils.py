...
from app.commons.utilities import get_server_logger
...
class ChatCompletionResponse(BaseModel):
    choices: List[Choice]
    model: Optional[str] = "Curve-Function"
...
async def chat_completion(req: ChatMessage, res: Response):
    ...
    tools_encoded = const.curve _function_hanlder._format_system(req.tools)
    ...
    client_model_name = const.curve _function_client.models.list().data[0].id
    ...
    if const.PREFILL_ENABLED:
        ...
        if first_token_content != const.TOOL_CALL_TOKEN:
            ...
            extra_body = {
                **const.curve _function_generation_params,
                "continue_final_message": True,
                "add_generation_prompt": False,
            }
            pre_fill_resp = const.curve _function_client.chat.completions.create(
                messages=messages,
                model=client_model_name,
                stream=False,
    ...
    tool_calls = const.curve _function_hanlder.extract_tool_calls(full_response)
    ...
    logger.info(
        f"server <= curve _function: (tools): {json.dumps([tool_call['function'] for tool_call in tool_calls])}"
    )
    logger.info(
        f"server <= curve _function: response body: {json.dumps(chat_completion_response.dict())}"
    )
