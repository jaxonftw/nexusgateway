...
use common::common_types::open_ai::{
    CurveState, ChatCompletionTool, ChatCompletionsRequest, ChatCompletionsResponse, Choice,
...
use common::consts::{
    CURVE_FC_INTERNAL_HOST, CURVE_FC_MODEL_NAME, CURVE_FC_REQUEST_TIMEOUT_MS, CURVE_INTERNAL_CLUSTER_NAME, CURVE_MESSAGES_KEY, CURVE_MODEL_PREFIX, CURVE_STATE_HEADER, CURVE_UPSTREAM_HOST_HEADER, ASSISTANT_ROLE, CHAT_COMPLETIONS_PATH, DEFAULT_EMBEDDING_MODEL, DEFAULT_HALLUCINATED_THRESHOLD, DEFAULT_INTENT_MODEL, DEFAULT_PROMPT_TARGET_THRESHOLD, EMBEDDINGS_INTERNAL_HOST, GPT_35_TURBO, GUARD_INTERNAL_HOST, HALLUCINATION_INTERNAL_HOST, REQUEST_ID_HEADER, SYSTEM_ROLE, TOOL_ROLE, USER_ROLE, ZEROSHOT_INTERNAL_HOST
};
...
pub struct StreamContext {
    ...
    curve _state: Option<Vec<CurveState>>,
...
impl StreamContext {
    ...
    fn zero_shot_intent_detection_resp_handler(
        ...
        let msg_body = match serde_json::to_string(&chat_completions) {
            ...
            Err(e) => {
                debug!("error serializing curve _fc request body: {}", e);
    ...
    fn function_resolver_handler(&mut self, body: Vec<u8>, mut callout_context: StreamCallContext) {
        ...
        debug!("curve  <= app response body: {}", body_str);
        ...
        let curve _fc_response: ChatCompletionsResponse = match serde_json::from_str(&body_str) {
            ...
            Err(e) => {
                debug!("error deserializing curve _fc response: {}", e);
                return self.send_server_error(ServerError::Deserialization(e), None);
            }
        };
    ...
    fn function_call_response_handler(
        ...
        debug!("curve  => upstream llm request body: {}", json_string);

        self.set_http_request_body(0, self.request_body_size, &json_string.into_bytes());
        self.resume_http_request();
    }

    fn curve _guard_handler(&mut self, body: Vec<u8>, callout_context: StreamCallContext) {
            ...
            if let Some(tool_calls) = self.tool_calls.as_ref() {
                if !tool_calls.is_empty() {
                    ...
                    if let Value::Object(ref mut map) = data {
                        ...
                        // since curve  gateway generates tool calls (using curve -fc) and calls upstream api to
                        // get response, we will send these back to developer so they can see the api response
                        // and tool call curve -fc generated
                        ...
                        debug!("curve  => user: {}", data_serialized);
