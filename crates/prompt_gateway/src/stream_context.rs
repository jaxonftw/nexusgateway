...
use common::consts::{
    CURVE_FC_MODEL_NAME, CURVE_FC_REQUEST_TIMEOUT_MS, CURVE_INTERNAL_CLUSTER_NAME, CURVE_MESSAGES_KEY,
    CURVE_MODEL_PREFIX, CURVE_STATE_HEADER, CURVE_UPSTREAM_HOST_HEADER, ARC_FC_CLUSTER,
...
impl StreamContext {
    ...
    fn zero_shot_intent_detection_resp_handler(
        ...
        let mut headers = vec![
            (":method", "POST"),
            (CURVE_UPSTREAM_HOST_HEADER, ARC_FC_CLUSTER),
        ...
        if self.streaming_response {
            ...
            if let Some(tool_calls) = self.tool_calls.as_ref() {
                if !tool_calls.is_empty() {
                    ...
                    // push tool call state to curve  state
                    self.curve _state
                        .as_mut()
                        .unwrap()
                        .push(CurveState::ToolCall(vec![tool_call_state]));
                    ...
                    if let Value::Object(ref mut map) = data {
                        ...
                        let curve _state_str = serde_json::to_string(&self.curve _state).unwrap();
                        debug!("curve _state: {}", curve _state_str);
                        let metadata = map
                            .entry("metadata")
