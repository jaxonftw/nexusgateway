...
use common::common_types::open_ai::{
    to_server_events, CurveState, ChatCompletionStreamResponse, ChatCompletionTool,
...
use common::consts::{
    CURVE_FC_INTERNAL_HOST, CURVE_FC_MODEL_NAME, CURVE_FC_REQUEST_TIMEOUT_MS,
    CURVE_INTERNAL_CLUSTER_NAME, CURVE_MODEL_PREFIX, CURVE_STATE_HEADER, CURVE_UPSTREAM_HOST_HEADER,
...
impl StreamContext {
    ...
    pub fn embeddings_handler(&mut self, body: Vec<u8>, mut callout_context: StreamCallContext) {
        ...
        let mut headers = vec![
            (CURVE_UPSTREAM_HOST_HEADER, ZEROSHOT_INTERNAL_HOST),
            (":method", "POST"),
            (":path", "/zeroshot"),
    ...
    fn trace_curve _internal(&self) -> bool {
    ...
    pub fn hallucination_classification_resp_handler(
        ...
        debug!("curve <= hallucination response: {}", body_str);
    ...
    pub fn zero_shot_intent_detection_resp_handler(
            ...
            || curve _assistant
        {
            ...
            if curve _assistant {
                info!("curve  fc is engaged in parameter collection");
                ...
                .find(|pt| pt.default.unwrap_or(false))
            {
                ...
                let messages = self.filter_out_curve _messages(&callout_context);
                ...
                debug!(
                    "curve (low similarity score) => llm request: {}",
                    llm_request_str
                );
        ...
        // curve fc handler needs state so it can expand tool calls
        let mut metadata = HashMap::new();
        metadata.insert(
            CURVE_STATE_HEADER.to_string(),
            serde_json::to_string(&self.curve _state).unwrap(),
        );
        ...
        let msg_body = match serde_json::to_string(&chat_completions) {
            ...
            Err(e) => {
                warn!("error serializing curve _fc request body: {}", e);
                return self.send_server_error(ServerError::Serialization(e), None);
            }
        };
        ...
        let mut headers = vec![
            (":method", "POST"),
            (CURVE_UPSTREAM_HOST_HEADER, CURVE_FC_INTERNAL_HOST),
            (":path", "/v1/chat/completions"),
            (":authority", CURVE_FC_INTERNAL_HOST),
            ("content-type", "application/json"),
            ("x-envoy-max-retries", "3"),
            ("x-envoy-upstream-rq-timeout-ms", timeout_str.as_str()),
        ];
        ...
        callout_context.response_handler_type = ResponseHandlerType::CurveFC;
        callout_context.prompt_target_name = Some(prompt_target.name);

        debug!("curve => curve fc request: {}", msg_body);
        if let Err(e) = self.http_call(call_args, callout_context) {
            debug!("error dispatching curve _fc request: {}", e);
    ...
    pub fn curve _fc_response_handler(
        ...
        let curve _fc_response: ChatCompletionsResponse = match serde_json::from_str(&body_str) {
            ...
            Err(e) => {
                warn!("error deserializing curve fc response: {}", e);
                return self.send_server_error(ServerError::Deserialization(e), None);
            }
        };
        ...
        let mut headers = vec![
            (CURVE_UPSTREAM_HOST_HEADER, HALLUCINATION_INTERNAL_HOST),
            (":method", "POST"),
            (":path", "/hallucination"),
    ...
    pub fn api_call_response_handler(&mut self, body: Vec<u8>, callout_context: StreamCallContext) {
        ...
        debug!("curve => llm request: {}", llm_request_str);
    ...
    pub fn curve _guard_handler(&mut self, body: Vec<u8>, callout_context: StreamCallContext) {
        let prompt_guard_resp: PromptGuardResponse = serde_json::from_slice(&body).unwrap();
        debug!(
            "curve <= curve guard response: {:?}",
    ...
    pub fn default_target_handler(&self, body: Vec<u8>, mut callout_context: StreamCallContext) {
        ...
        debug!("curve => (default target) llm request: {}", json_resp);
