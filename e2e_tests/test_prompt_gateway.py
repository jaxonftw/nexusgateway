...
from common import PROMPT_GATEWAY_ENDPOINT, get_curve _messages, get_data_chunks
...
def test_prompt_gateway(stream):
    ...
    if stream:
        ...
        assert response_json.get("model").startswith("Curve")
...
@pytest.mark.parametrize("stream", [True, False])
def test_prompt_gateway_curve _direct_response(stream):
    body = {
        "messages": [
            {
                "role": "user",
    ...
    if stream:
        ...
        assert len(chunks) > 0
        response_json = json.loads(chunks[0])
        # make sure curve  responded directly
        assert response_json.get("model").startswith("Curve")
