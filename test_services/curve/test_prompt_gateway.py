...
def test_prompt_gateway(httpserver: HTTPServer):
    ...
    server_response = simple_fixture["server_response"]
    ...
    # now verify curve _messages (tool call and api response) that are sent as response metadata
    curve _messages = get_curve _messages(response_json)
    assert len(curve _messages) == 2
    tool_calls_message = curve _messages[0]
...
def test_prompt_gateway_server_500(httpserver: HTTPServer):
    simple_fixture = TEST_CASE_FIXTURES["SIMPLE"]
    input = simple_fixture["input"]
    ...
    assert (
        response.text
        == "upstream application error host=curve _internal, path=/function_calling, status=500, body="
    )
