...
def forward_to_llm(messages):
    ...
    # Use LangChain's LLM to get a response. This call is proxied through Curve for end-to-end observability and traffic management
    llm = OpenAI()
...
def process_rag():
    ...
    # Extract the intent change marker from Curve's headers if present for the current prompt
    intent_changed_header = request.headers.get('x-curve -intent-marker', '').lower()
    if intent_changed_header in ['', 'false']:
        ...
        return jsonify({'error': 'Invalid value for x-curve -prompt-intent-change header'}), 400
