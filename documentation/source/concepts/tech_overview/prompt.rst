...
build agentic applications using Curve, see our full guide :ref:`here <curve _agent_guide>`:

.. Note::
   Curve :ref:`Curve-Function <function_calling>` is the dedicated agentic model engineered in Curve to extract information from
   a (set of) prompts and executes necessary backend API calls. This allows for efficient handling of agentic tasks,
   such as scheduling data retrieval, by dynamically interacting with backend services. Curve-Function is a flagship 1.3
...
the OpenAI client and configure it with Curve. By sending traffic through Curve, you can propagate traces, manage and monitor
