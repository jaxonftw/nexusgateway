...
def start_curve (curve_config_file, env, log_timeout=120, foreground=False):
    """
    ...
    try:
        ...
        try:
            ...
            log.info("curve container found in docker, stopping and removing it")
            # ensure that previous docker container is stopped and removed
            container.stop()
            container.remove()
            log.info("Stopped and removed curve container")
        ...
        log.info("Keyboard interrupt received, stopping curve  gateway service.")
        stop_curve ()
...
def start_curve _modelserver(foreground):
    """
    ...
    try:
        ...
        if foreground:
            subprocess.run(
                ["curve_modelserver", "start", "--foreground"],
                check=True,
            )
        else:
            subprocess.run(
                ["curve_modelserver", "start"],
                check=True,
            )
    except subprocess.CalledProcessError as e:
