...
fn request_headers_expectations(module: &mut Tester, http_context: i32) {
    module
        ...
        .expect_get_header_map_value(
            ...
            Some("x-curve -deterministic-provider"),
        )
        .returning(Some("true"))
...
{\
    ...
    module
        ...
        .expect_http_call(Some("curve_server"), None, None, None, None)
        .returning(Some(1))
        .expect_log(Some(LogLevel::Debug), None)
    ...
    module
        ...
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Info), None)
        .expect_http_call(Some("curve _fc"), None, None, None, None)
        .returning(Some(3))
        .expect_log(Some(LogLevel::Debug), None)
        .expect_log(Some(LogLevel::Debug), None)
        .expect_metric_increment("active_http_calls", 1)
...
fn request_ratelimited() {
    ...
    module
        .call_proxy_on_http_call_response(http_context, 3, 0, curve _fc_resp_str.len() as i32, 0)
        .expect_metric_increment("active_http_calls", -1)
        .expect_get_buffer_bytes(Some(BufferType::HttpCallResponseBody))
