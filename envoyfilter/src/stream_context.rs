...
use crate::common_types::Securve PointsResponse;
...
impl StreamContext {
    ...
    fn embeddings_handler(&mut self, body: Vec<u8>, mut callout_context: CallContext) {
        ...
        let securve _points_request = common_types::Securve PointsRequest {
        ...
        let json_data: String = match serde_json::to_string(&securve _points_request) {
            ...
            Err(e) => {
                warn!("Error serializing securve _points_request: {:?}", e);
    ...
    fn securve _points_handler(&mut self, body: Vec<u8>, mut callout_context: CallContext) {
        let securve _points_response: Securve PointsResponse = match serde_json::from_slice(&body) {
            ...
            Err(e) => {
                warn!("Error deserializing securve _points_response: {:?}", e);
                self.resume_http_request();
                return;
            }
        };
        ...
        if securve _results[0].score < DEFAULT_PROMPT_TARGET_THRESHOLD {
            info!(
...
impl Context for StreamContext {
    fn on_http_call_response(
        ...
        match callout_context.request_type {
            ...
            RequestType::Securve Points => {
                self.securve _points_handler(body, callout_context);
            }
            RequestType::Ner => {
