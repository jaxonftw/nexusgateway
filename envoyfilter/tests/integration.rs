...
use public_types::{
    common_types::{self, NERResponse, Securve PointResult, Securve PointsResponse},
...
fn normal_flow(module: &mut Tester, filter_context: i32, http_context: i32) {
    ...
    module
        ...
        .expect_get_header_map_value(
            Some(MapType::HttpRequestHeaders),
            Some("x-curvelaboratory-ratelimit-selector"),
        )
...
{\
    ...
    let securve _points_response = Securve PointsResponse {
        status: String::new(),
        time: 0.0,
        result: vec![Securve PointResult {
            id: String::new(),
            version: 0,
            score: 0.7,
    ...
    let securve _points_response_buffer = serde_json::to_string(&securve _points_response).unwrap();
    module
        .call_proxy_on_http_call_response(
            http_context,
            2,
            0,
            securve _points_response_buffer.len() as i32,
            0,
        )
        .expect_metric_increment("active_http_calls", -1)
        .expect_get_buffer_bytes(Some(BufferType::HttpCallResponseBody))
        .returning(Some(&securve _points_response_buffer))
